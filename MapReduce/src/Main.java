...
public class Main {
    ...
    public static void main(String[] args) throws ExecutionException {
        ...
        // Create an array of threads
        PiClass piClass[] = new PiClass[numThreads];
        ArrayList<PiClass> piClasses = new ArrayList<>();

        //Execute all threads. PiThread gets a number-id of thread(i), the number of threads, the shared array and numSteps
        for (int i = 0; i < numThreads; i++) {
            piClass[i] = new PiClass(i, numThreads, a, numSteps);
            piClasses.add(piClass[i]);
        }

        //use map-reduce to calculate pi (with piclass.Execute())
        Double pi = piClasses.stream()
                .map(p -> p.Execute())
                .reduce(0.00,(c,b) -> c+b);
